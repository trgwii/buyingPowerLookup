 ...
 */
 ...
 if(res.status > 200){
     ...
     for (let tType = 0; tType <= 1; tType++) {
        const reqEndpoint = '/sapi/v1/fiat/orders';
        const cacheFile = `./cache${reqEndpoint}/${tType}.json`;
        const now = Date.now();
        const query = `timestamp=${now.toString()}&transactionType=${tType}`
        const signature = hmac("sha256", secretKey, query, "utf8", "hex");
