...
const allTrades = binanceDB.query('SELECT tradeID, symbol, time, side, price FROM trade');
for (const tradeData of allTrades) {
    ...
    if(quoteAsset === fiatCurrency.toUpperCase()){
        binanceDB.query(`INSERT OR IGNORE INTO tradeValue (
                id,
                type,
                value
            ) VALUES ( ?, ?, ?)`,
